...
with st.expander("帳上毛利率 vs 集團淨利率 差在哪？（點開看說明）"):
    st.markdown(
        "兩者**分母都是除佣實收**，差別在分子——這是系統的「三層毛利」：\n\n"
        "| 層 | 算法 | 白話 |\n"
        "|---|---|---|\n"
        "| ① 帳上毛利 | 除佣實收 − 實付成本 | 每家公司**帳面各自認列**的毛利 |\n"
        "| ② 集團毛利 | 帳上毛利 ＋ 加回公司間轉撥 | 母子公司互開（內部沖掉的）**還原成集團視角** |\n"
        "| ③ 淨利 | 集團毛利 − 固定成本 | 再扣**人事、租金等固定成本** |\n\n"
        "- **帳上毛利率** ＝ ① ÷ 除佣實收：看**單一公司帳面**賺頭。\n"
        "- **集團淨利率** ＝ ③ ÷ 除佣實收：看**整個集團扣掉所有成本後真正落袋**的比率。\n\n"
        "兩者差在兩步：**帳上→集團**加回公司間轉撥；**集團→淨利**再扣固定成本。\n\n"
        "> ⚠️ 目前 2025 固定成本規則尚未設定，**淨利暫等於集團毛利**，"
        "所以此處「集團淨利率」其實還沒扣固定成本（＝集團毛利率）；待固定成本規則補上後才會低於集團毛利率。"
    )
